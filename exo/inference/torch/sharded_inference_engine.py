    ...
    def load_medusa():
      try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            use_cache=True  # Needed for generating hidden states used by Medusa
        )
        
        # Set model to evaluation mode
        model.eval()
        
        # CRITICAL: Check if model has .transformer attribute for accessing hidden states
        if not hasattr(model, 'transformer') and not hasattr(model, 'model'):
          print("WARNING: Model does not have expected structure for Medusa")
          
        # Create Medusa heads here - these are the additional prediction heads needed for speculative decoding
        # NOTE: This is a critical step that was missing before
        import torch.nn as nn
        
        # Determine which attribute stores the model structure
        model_attr = 'transformer' if hasattr(model, 'transformer') else 'model'
        
        # Get vocabulary size and hidden size from model config
        ...
        # Create the Medusa heads
        try:
          # Create multiple heads that predict tokens at different future positions
          medusa_heads = nn.ModuleList([nn.Linear(hidden_size, vocab_size) for _ in range(4)])
          
          # Move heads to the same device as the model
          device = next(model.parameters()).device
          medusa_heads = medusa_heads.to(device)
          
          # Store the heads directly on the model
          model.medusa_heads = medusa_heads
          
          if DEBUG >= 1:
            print(f"Successfully created {len(medusa_heads)} Medusa heads")
            print(f"Model now has medusa_heads attribute: {hasattr(model, 'medusa_heads')}")
        except Exception as e:
          print(f"ERROR creating Medusa heads: {e}")
        
        if DEBUG >= 1:
          print(f"Medusa model successfully loaded")
          print(f"Model type: {type(model).__name__}")
          print(f"Model has medusa_heads: {hasattr(model, 'medusa_heads')}")
        ...
        # Get probability threshold from environment if available
        import os
        probability_threshold = os.environ.get('MEDUSA_PROBABILITY_THRESHOLD')
        debug_mode = os.environ.get('MEDUSA_DEBUG', '').lower() in ('true', '1', 't', 'yes')
        force_test = os.environ.get('MEDUSA_FORCE_TEST', '').lower() in ('true', '1', 't', 'yes')
        
        # Default threshold (if not provided in environment)
        ...
        if probability_threshold is not None:
          try:
            ...
            if DEBUG >= 1 or debug_mode:
              print(f"\n✅ Setting Medusa probability threshold to {threshold_value} from environment variable")
          except ValueError:
            print(f"Invalid MEDUSA_PROBABILITY_THRESHOLD value: {probability_threshold}, using default 0.5")
        
        # Create a reference to the Medusa heads on the model
        # This makes them accessible to the decoder
        if hasattr(model, 'medusa_heads'):
          # We'll store this for use by the decoder
          medusa_model_heads = model.medusa_heads
          if DEBUG >= 1 or debug_mode:
            print(f"Found {len(medusa_model_heads)} Medusa heads on model")
        else:
          # If the model doesn't have heads, create them here
          # This is a fallback for models that don't come with heads
          print("WARNING: No Medusa heads on model, creating them dynamically")
          medusa_model_heads = None
        ...
        medusa_decoder = MedusaDecoder(
          ...
          probability_threshold=threshold_value,  # Set threshold from environment
          debug=debug_mode,  # Enable debug mode from environment
          model_medusa_heads=medusa_model_heads  # Pass the actual heads to the decoder
        )
        
        if DEBUG >= 1 or debug_mode:
          print(f"Medusa decoder created with:")
          print(f"  - probability_threshold: {threshold_value}")
          print(f"  - debug mode: {debug_mode}")
          print(f"  - force test: {force_test}")
          print(f"  - medusa_heads: {medusa_config['heads']}")
          print(f"  - tree_size: {medusa_config['tree_size']}")
          print(f"  - max_candidates: {medusa_config['candidates']}")
  ...
  async def _medusa_infer_prompt(self, request_id: str, shard: Shard, prompt: str, inference_state: Optional[dict] = None) -> tuple[np.ndarray, Optional[dict]]:
    ...
    def generate_with_medusa():
      try:
        ...
        import os
        probability_threshold = os.environ.get('MEDUSA_PROBABILITY_THRESHOLD', '0.99999') # Default to extremely high threshold
        debug_mode = os.environ.get('MEDUSA_DEBUG', '').lower() in ('true', '1', 't', 'yes')
        force_filter = os.environ.get('MEDUSA_FORCE_FILTER', '').lower() in ('true', '1', 't', 'yes')
        
        # Print debug information about Medusa settings - always show this
        print(f"\n==== MEDUSA ENGINE SETTINGS =====")
        print(f"Probability threshold: {probability_threshold}")
        print(f"Debug mode: {'ENABLED' if debug_mode else 'DISABLED'}")
        print(f"Force filtering: {'ENABLED' if force_filter else 'DISABLED'}")
        
        # Apply threshold if provided
        if probability_threshold:
          try:
            probability_threshold = float(probability_threshold)
            if hasattr(self.medusa_decoder, 'probability_threshold'):
              # Set threshold on decoder
              self.medusa_decoder.probability_threshold = probability_threshold
              print(f"\n✅ Set probability threshold to {probability_threshold}")
              
              # Set force filtering directly on decoder
              if hasattr(self.medusa_decoder, 'force_filtering'):
                self.medusa_decoder.force_filtering = force_filter
                if force_filter:
                  print(f"⚠️ FORCE FILTERING IS ENABLED - Will artificially filter tokens")
          except ValueError:
            print(f"Invalid MEDUSA_PROBABILITY_THRESHOLD value: {probability_threshold}")
        
        # CRITICAL CHANGE: Use a much higher temperature to create more varied probabilities that
        # can be properly filtered by our threshold. The default of 0.0 leads to all probabilities
        # being very close to 1.0, which is why nothing gets filtered even with a high threshold.
        temperature = 1.2  # Higher temperature increases randomness and creates more varied probabilities
        top_p = 0.5        # More aggressive top_p filtering creates more varied probability distributions
        
        print(f"\n⚙️ Using temperature={temperature} and top_p={top_p} for varied probabilities")
        print(f"This creates natural probability variation that can be filtered by threshold={probability_threshold}")
        print(f"\n✅ Starting Medusa generation with threshold {self.medusa_decoder.probability_threshold}")
        
        generated_ids = self.medusa_decoder.generate(
          input_tokens,
          max_new_tokens=max_new_tokens,
          temperature=temperature,
          top_p=top_p  # Also pass the top_p parameter
        )
